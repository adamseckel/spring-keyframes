...
import { calcAngularFreq, findSpring } from "./findSpring"
...
export interface Spring {
  ...
  forEachFrame: (cb: (value: number, index: number, done: boolean) => void, withEveryFrame: boolean) => void
}

/** Popmotion's `spring.ts` */
export function createSpring({ from = 0.0, to = 1.0, restSpeed = 2, restDelta, ...options }: SpringOptions): Spring {
  /**
   * This is the Iterator-spec return value. We ensure it's mutable rather than using a generator
   * to reduce GC during animation.
   */

  let { stiffness, damping, mass, velocity, isResolvedFromDuration } = getSpringOptions(options)
  const state = { done: false, value: from, velocity }
  ...
  return {
    resolveValue: resolveSpring,
    resolveVelocity,
    forEachFrame: (callback: (value: number, duration: number, done: boolean) => void, withEveryFrame: boolean) => {
      let done = false
      let lastVelocity: number = 0
      let index = 0

      while (!done) {
        const frame = next(msPerFrame * index)
        ...
        if (withEveryFrame || isMaxAmplitude || index === 0 || done) callback(frame.value, index, done)
